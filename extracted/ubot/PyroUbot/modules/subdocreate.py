...
async def subdomain_create(client, message):
    ...
    if domain not in DOMAIN_LIST:
        await message.reply_text(f"⌭ Domain `{domain}` ga ketemu dalam daftar. Gunakan `.domainlist` untuk melihat daftar domain yang tersedia.")
        return
    ...
    if result.get("success"):
        await message.reply_text(f"⌬ **Subdomain berhasil ditambah!**\n◎ `{full_subdomain} → {target_ip}`")
    else:
