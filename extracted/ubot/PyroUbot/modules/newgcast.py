...
async def _(c, m):
    ...
    if len(m.command) != 2:
        await m.reply(f"**<emoji id =5929358014627713883>⌭</emoji> tolong gunakan format: bc [gc adm pv] **")
        return
    send = get_message(m)
    if not send:
        await m.reply_text(f"<emoji id=5911461474315802019>✦</emoji> **tolong balas ke pesan** !", quote=True)
        return
    ...
    try:
        ...
        await m.reply(f"<emoji id =5929358014627713883>⌭</emoji>**tolong gunakan bc gc/adm/pv balas ke pesan**")
