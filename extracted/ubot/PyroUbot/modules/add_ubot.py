...
async def add_ubot_callback(client, callback_query):
    ...
    bot_msg = await bot.send_message(
        user_id,
        "<blockquote><b>⚙️ Lagi proses aktivasi, tunggu sebentar ya...</b></blockquote>",
    )
    ...
    await bot_msg.edit(
        ...
        f"<b>Join channel:</b> @ZexcOfficiall biar tetap update!\n"
...
async def cek_masa_aktif_callback(client, callback_query):
    ...
    try:
        ...
        return await callback_query.answer("✅ Tidak ada masa aktif / sudah expired", show_alert=True)
...
async def del_ubot_callback(client, callback_query):
    ...
    for X in ubot._ubot:
        if get_id == X.me.id:
            ...
            await callback_query.answer(f"✅ {get_mention} berhasil dihapus!", show_alert=True)
            await callback_query.edit_message_text(
                await MSG.UBOT(0),
                reply_markup=InlineKeyboardMarkup(BTN.UBOT(ubot._ubot[0].me.id, 0)),
            )
...
async def tools_userbot_callback(client, callback_query):
    ...
    if query[0] == "get_otp":
        async for otp in X.search_messages(777000, limit=1):
            try:
                if not otp.text:
                    await callback_query.answer("❌ Kode OTP tidak ditemukan", show_alert=True)
                else:
                    await callback_query.edit_message_text(
                        otp.text,
        ...
        if code is None:
            return await callback_query.answer("🔐 Kode 2FA tidak ditemukan", show_alert=True)
        return await callback_query.edit_message_text(
