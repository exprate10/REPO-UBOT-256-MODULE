...
async def get_shio(client, message):
    ...
    API_URL = f"https://api.botcahx.eu.org/api/primbon/shio?shio={shio}&tanggal={tanggal}&bulan={bulan}&tahun={tahun}&apikey=@31Moire_mor"
    ...
    try:
        ...
        if not data.get("status") or not data["result"].get("status"):
            return await message.reply_text("⌯ Data ga ketemu atau terjadi kesalahan.")
