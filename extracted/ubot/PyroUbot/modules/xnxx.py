...
async def random_bokep(client, message):
    try:
        ...
        api_url = f"https://api.botcahx.eu.org/api/search/xnxx?query={search_query}&apikey=@31Moire_mor"
        ...
        if not results:
            await status_msg.edit(f"<emoji id=5215204871422093648>⌭</emoji> ga ketemu hasil untuk: **{search_query}**")
            return
        ...
        dl_url = f"https://api.botcahx.eu.org/api/download/xnxxdl?url={data['link']}&apikey=@31Moire_mor"
